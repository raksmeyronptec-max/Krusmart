...
/** Assessments for the active year, with their class and subject resolved. */
...
export async function getAssessments(scope: AdminScope): Promise<AssessmentRow[]> {
  const supabase = await createClient()
  const { data } = await supabase
    .from('assessments')
    .select('id, name, type, max_score, weight, term, date, class_subjects(classes(name), subjects(name))')
    .eq('academic_year_id', scope.activeYear?.id ?? '')
    .order('date', { ascending: false })

  const rows = (data ?? []) as unknown as {
    id: string; name: string; type: string; max_score: number; weight: number
    term: string | null; date: string | null
    class_subjects?: {
      classes?: { name?: string } | { name?: string }[]
      subjects?: { name?: string } | { name?: string }[]
    } | { classes?: unknown; subjects?: unknown }[]
  }[]
  ...
  return rows.map((a) => {
    const cs = one(a.class_subjects) as
      | { classes?: { name?: string } | { name?: string }[]; subjects?: { name?: string } | { name?: string }[] }
      | undefined
    return {
      id: a.id,
      name: a.name,
      ...
      term: a.term,
      date: a.date,
      className: one(cs?.classes)?.name ?? '—',
      subjectName: one(cs?.subjects)?.name ?? '—',
    }
  })
}

/** class_subject pairs, for the assessment-creation form. */
export interface ClassSubjectOption {
  id: string
  label: string
}

export async function getClassSubjectOptions(scope: AdminScope): Promise<ClassSubjectOption[]> {
  const supabase = await createClient()
  const { data } = await supabase
    .from('class_subjects')
    .select('id, classes!inner(name, academic_year_id), subjects(name)')
    .eq('classes.academic_year_id', scope.activeYear?.id ?? '')
  ...
  return rows
    .map((r) => ({
      id: r.id,
      label: `${one(r.classes)?.name ?? '—'} › ${one(r.subjects)?.name ?? '—'}`,
    }))
    .sort((a, b) => a.label.localeCompare(b.label, 'km'))
}
