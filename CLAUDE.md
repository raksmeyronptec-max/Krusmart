...
**The unit of layout is the errand, not the category.** The centre listed nine families as nine equal panels — a catalogue, which is the wrong shape because a teacher does not arrive with a category in mind, they arrive with this month's job. `REPORT_SECTIONS` in [report-types.ts](lib/reporting/report-types.ts) groups the nine categories into five *shelves* by errand (របាយការណ៍ផ្លូវការ · វត្តមាន · លទ្ធផលប្រចាំឆ្នាំ · រៀបចំថ្នាក់រៀន · ឯកសារសិស្ស); a shelf's `groups` name the runs inside it, and `ReportDefinition.group` subdivides the seven-report yearly family further. All of it is declared in the catalogue, because a second list of groupings inside `PrintCenterClient` is exactly the drift the catalogue exists to prevent.

**Those five shelf headings are the page's ONLY navigation level.** A first pass also put a row of five shelf chips above them — both real navigation, which made them competing navigation: the chips named the five destinations and the very next thing on the page named them again, in the same order, as headings that could actually be opened. Stacked under បោះពុម្ពឆាប់ៗ, a period bar and a search box, the upper half was four rows of outlined controls before one document appeared. The chips are gone; the headings navigate. Categories stay canonical and `?category=student` / `?category=certificate` still resolve, now narrowing to the one shelf that holds them, opened, with a visible way back — category filtering is a **secondary** interaction reached by a link, never a second permanent row.

Three things above the shelves answer "what am I printing?" before any list is read:

- **`បោះពុម្ពឆាប់ៗ`** — `QUICK_ACTION_REPORTS`, a fixed four, each resolved against the period bar. Every one must generate (`verify-reporting.mts` pins it): the biggest buttons on the page may not say កំពុងរៀបចំ. They are **ranked, not four equal tiles** — `emphasis` declares one `primary` (the month's marks, filled and wider), two `secondary` and one `quiet` (the year's totals, which belong here so they are findable in October and are noise in February). Each carries a short `label` naming the *errand* (`ពិន្ទុ`), not the document (`តារាងពិន្ទុប្រចាំខែ`) — and deliberately not `ពិន្ទុខែនេះ`, because the card's second line is the real resolved period and "this month" above `ខែធ្នូ ២០២៥` is a lie the moment the period bar moves.
...
- **Progressive disclosure** — a shelf is open when it holds a P0 document (`PRIMARY_DOCUMENTS`) that reads the rung the period bar is on, so September opens ពិន្ទុ and វត្តមាន and leaves the seven annual rows shut, and moving the bar to ឆ្នាំ opens the year's shelf. That is *derived*, not seeded, or switching rung leaves the wrong shelf open and the right one shut. `overrides` is what stops it being the page arguing: a shelf the teacher has opened or closed by hand is never reconsidered. A closed shelf names its runs and counts its documents.
...
**A document says what it will contain before it is made.** `៣៥ សិស្ស · ៨ មុខវិជ្ជា` is compatible with a completely blank grid, which is the sheet a teacher then hands to a class. [readiness.ts](lib/reporting/readiness.ts) (pure) grades the payload the resolver just built — `filledRowCount` in [report-mapper.ts](lib/reporting/report-mapper.ts) counts the rows that would print something, ignoring the name-and-number columns every row carries whether or not the pupil was ever assessed. It **never blocks generation**: an empty register printed to fill in by hand and a part-marked month are both existing product semantics, stated rather than changed.

Two facts about each report make that judgement honest, and both are declared in the catalogue because both were got wrong by assuming:

- **`rowBasis(type)`** — `honor`, `certificate` and the two promotion lists print the pupils who **qualified**, not the roster. Judged against the roster, a fully marked class of thirty was told by its own honour roll that it had no marks, and by its promotion list to go and enrol some pupils. An empty one now reads `មិនមានសិស្សចូលលក្ខណៈ` and offers no fix, because an empty honour roll is not a fault. `tpp_master_book` is `other`: its data rides in `meta.tppMaster` and its `rows` is always empty, so nothing can be read off it — silence is the only honest output.
- **`hasSubjectRegion(type)`** — the certificate is a page of prose and the three fixed annual tables have no subject columns, so their `subjectCount === 0` is permanent and correct. Read as "configure your subjects" it sent teachers to `/score/subjects` to fix a document that has never had one.

**`scripts/report-inventory.mts` is the roadmap**, derived from the catalogue and the template registry so it cannot drift: `npx tsx scripts/report-inventory.mts` (or `--json`). Its six states are a renaming of `reportAvailability`'s four plus the two the catalogue cannot express — `MISSING` (a planned document with nothing behind it) and `NOT_APPLICABLE` (a printable screen deliberately outside the index, read out of `verify-students.mts` rather than restated). `verify-reporting.mts` asserts the renaming, so there is no second availability model.

**The preview states only measured facts.** `សិស្ស · មុខវិជ្ជា` come from the resolver that built that very sheet; `A4` and `ផ្ដេក` are read back out of the filled workbook's own `pageSetup`, so a template declaring none shows none. There is deliberately **no page count**: pagination depends on the printer, the driver and the scale, and an invented `៤ ទំព័រ` is worse than silence because it looks checkable.

**The flow is a rail, not a wizard:** ឯកសារ → រយៈពេល → ជម្រើស → ពិនិត្យ → ទាញយក, all on one panel, with ជម្រើស folded away unless it holds something required. **រយៈពេល is answered on arrival, so it is stated and not asked** — the period bar already made the choice, and repeating it as an open dropdown asked a second time for something the teacher had said; `ប្តូររយៈពេល` opens the selector for the teacher who genuinely wants this one document for another month. The stated phrase comes from `resolvePeriod`, so the row that was pressed and the dialog that opened cannot name the period differently. It defaults to `activeTemplate(type)` and not `templatesFor(type)[0]` — the latter is newest-version-first and will happily pick a version marked `isActive: false`, which made the index's row and the dialog disagree about which file the button produces. It says **ទាញយក**, never បោះពុម្ព: the engine writes a file, and the screens are what print.
