 ...
 *   legacy / loading   renders nothing. A pre-V2 account has no class entity at
 *                      all, and inventing a strip for one would be a label over
 *                      an absence.
 ...
 */
export function ClassContextBar({ className = '' }: { className?: string }) {
  ...
  if (loading || isLegacy || !classLabel) return null
  ...
  const yearLabel = academicYearName ? `ឆ្នាំសិក្សា ${toKhmerNumber(academicYearName)}` : null
  ...
  return (
    ...
    >
      ...
      */}
      ...
      <span className="kh-truncate font-bold text-text-heading">{classLabel}</span>
      ...
      */}
      ...
      {yearLabel && (
        <>
          <span aria-hidden="true" className="hidden text-text-muted sm:inline">·</span>
          <span className="text-text-muted">{yearLabel}</span>
        </>
      )}
